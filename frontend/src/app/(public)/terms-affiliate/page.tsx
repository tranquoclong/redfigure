...
async function getPage() {
  try {
    const { data } = await api.get("/pages/terms-affiliate");
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!page) return { title: "Điều khoản chương trình liên kết" };
  return buildPageMetadata({
    title: page.metaTitle || page.title,
    description:
      page.metaDescription ??
      "Điều khoản và điều kiện của chương trình liên kết Red Figure.",
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16">
      <h1 className="text-4xl md:text-5xl font-black mb-2 tracking-wide text-white">
        {page?.title ?? "Điều khoản chương trình liên kết"}
      </h1>
      ...
      {page?.content ? (
        ...
        <p className="text-white/60">
          Nội dung sớm có. Nếu bạn cần tham gia chương trình, vui lòng liên hệ
          với bộ phận hỗ trợ.
        </p>
      )}
    </div>
